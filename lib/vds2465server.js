...
class vds2465server extends eventhandler{
    ...
    */
    addDevice(identnr, stehend, keynr, key = '') {
        try {
            ...
            this.#logging(`Geraet angelegt: ${JSON.stringify(obj)}`, 'debug');
        }
        catch (e) {
    ...
    */
    #controller(func) {
        ...
        switch (func) {
            ...
            case _action.IK3_nach_Pollzeit:
                ...
                this.#Timer2 = setTimeout(this.#controller.bind(this), this.#Polling, _action.IK3);
                break;
    ...
    #timer() {
        ...
        this.#Timer = setTimeout(this.#controller.bind(this), this.#Polling + 1000, _action.Timer_abgelaufen);
    }
    ...
    #sendIK2() {
        ...
        buf = this.#Telegramm_Zusatz(buf);
        this.#logging('out IK2: ' + buf.toString('hex'), 'silly');
        this.#send(buf);
    }
    ...
    //Satz02 Meldung als Buffer
    #getMeldungBuffer(Meldung) {
        let buf = Buffer.alloc(7);
        let offset = 0, wert, index;
        let Msg = {
            Li: 0,
            Ge: 0,
            Be: 0,
        ...
        //Text = "";
        let Werte = Meldung.split(';');
        Werte.forEach(function (data) {
            if (data.length > 2) {
                switch (data.substring(0, 3)) {
                    case 'Li:': wert = parseInt(data.substring(3), 10);
                        if (wert >= 0 && wert <= 255) {
                            Msg.Li = wert;
                        }
                        break;
                    case 'Ge:': wert = parseInt(data.substring(3), 10);
                        if (wert >= 0 && wert <= 255) {
                            Msg.Ge = wert;
                        }
                        break;
                    case 'Be:': wert = parseInt(data.substring(3), 10);
                        if (wert >= 0 && wert <= 255) {
                            Msg.Be = wert;
                        }
                        break;
                    case 'Az:': wert = parseInt(data.substring(3), 10);
                        if (wert >= 0 && wert <= 255) {
                            Msg.Az = wert;
                        }
                        break;
                    case 'Ma:': wert = parseInt(data.substring(3), 10);
                        if (wert >= 0 && wert <= 255) {
                            Msg.Ma = wert;
                        }
                        break;
                    case 'Art:': wert = parseInt(data.substring(4), 10);
                        if (wert > 0 && wert <= 10) {
                            Msg.Art = wert;
                        }
                        break;
                    default: index = Meldung.indexOf('Text:"', 0);
                        if (index >= 0) {
                            let end = Meldung.indexOf('"', index + 6);
                            Text = Meldung.slice(index + 6, end);
                        }
                        break;
                }
            }
        ...
        buf.writeUInt8(((Msg.Ge << 4) | Msg.Be) & 0xFF, offset++);
        buf.writeUInt8(Msg.Li, offset++);
        buf.writeUInt8(Msg.Az, offset++);
        buf.writeUInt8(Msg.Art, offset++);
        buf.writeUInt8(Msg.Ma, offset++);
        //log(JSON.stringify(Msg), 'debug');
        //log('Meldung aufbereitet: ' + buf.toString('hex'), 'debug');
        return buf;
    }
    ...
    */
    ...
    //Satz51 Hersteller als Buffer
    #getHerstellerBuffer(Text) {
        if ((typeof Text != 'string') || Text.length === 0) {
            //log('getTextBuffer: kein Text', 'debug');
            return Buffer.alloc(0);
        }
        let buf1 = Buffer.alloc(2);
        let offset = buf1.writeUInt8(Text.length, 0);
        offset = buf1.writeUInt8(0x51, offset);
        const buf2 = Buffer.from(Text);
        const buf3 = Buffer.concat([buf1, buf2], 2 + Text.length);
        //log('Hersteller als buffer: ' + buf3.toString('hex'), 'debug');
        return buf3;
    }

    //Satz54 Text als Buffer
    #getTextBuffer(Text) {
        if ((typeof Text != 'string') || Text.length === 0) {
            //log('getTextBuffer: kein Text', 'debug');
            return Buffer.alloc(0);
        ...
        let buf1 = Buffer.alloc(2);
        let offset = buf1.writeUInt8(Text.length, 0);
        offset = buf1.writeUInt8(0x54, offset);
        const buf2 = Buffer.from(Text);
        const buf3 = Buffer.concat([buf1, buf2], 2 + Text.length);
        //log('Text als buffer: ' + buf3.toString('hex'), 'debug');
        return buf3;
    }

    //Satz56 Identnummer als Buffer
    #getIdentnrBuffer(Id) {
        let buf;
        let laenge = 0;
        let ungerade = false;
        ...
        if ((typeof Id != 'string') || Id.length === 0) {
            //log("getIdentnrBuffer: parameter fehlt!", 'debug');
            return Buffer.alloc(0);
        }
        if ((Id.length % 2) === 0) //gerade
        {
            laenge = Id.length / 2;
        }
        else //ungerade
        {
            laenge = (Id.length / 2) + 1;
            ungerade = true;
        }
        buf = Buffer.alloc(laenge + 2);
        offset = buf.writeUInt8(laenge, 0);
        offset = buf.writeUInt8(0x56, offset);
        let nibbel_r, nibbel_l;
        for (let i = 0; i < Id.length - 1; i += 2) {
            nibbel_r = parseInt(Id.charAt(i)) & 0x0F;
            nibbel_l = parseInt(Id.charAt(i + 1));
            nibbel_l = nibbel_l << 4;
            nibbel_l = nibbel_l & 0xF0;
            offset = buf.writeUInt8(nibbel_l | nibbel_r, offset);
        }
        if (ungerade) {
            nibbel_r = parseInt(Id.charAt(Id.length - 1)) & 0x0F;
            offset = buf.writeUInt8(0xF0 | nibbel_r, offset);
        }
        //log('ID als buf: '+ buf.toString("hex"));
        return buf;
    }
    ...
    */
    ...
    //Satz61 Transportdienst
    #getTansportBuffer(weg) {
        if (!weg) {
            weg = 0x90;
        }
    ...
    //Satz10
    #decodiereSatz10(SL, data) {
        //log('decodiereSatz10:' + data.toString('hex'), 'debug');
        let offset = 0;
        if (SL < 2) {
            return;
        }
        let geraetbereich = data.readUInt8(offset++);
        let ge = geraetbereich >> 4 & 0x0F;
        let be = geraetbereich & 0x0F;
        let adrerw = 0, adr = 0, adrzus = 0, typ = 0;
        let buf;
        if (SL === 2) {
            typ = data.readUInt8(offset++);
        }
        else if (SL === 5) {
                adr = data.readUInt8(offset++);
                adrzus = data.readUInt8(offset++);
                adrerw = data.readUInt8(offset++);
                typ = data.readUInt8(offset++);
        }
        //offset = 0;
        switch (typ) {
            case 0x20:
                //const zustand = getStateAntwort(adr, adrerw);
                const zustand = -1;
                if (zustand < 0) {
                    buf = Buffer.alloc(4, 0);
                    buf.writeUInt8(0x02, offset++);
                    buf.writeUInt8(0x11, offset++);
                    buf.writeUInt8(geraetbereich, offset++);
                    buf.writeUInt8(0x10, offset++);  //Adresse nicht vorhanden, außerhalb des Bereichs
                }
                else {
                    buf = Buffer.alloc(7, 0);
                    buf.writeUInt8(0x05, offset++);
                    buf.writeUInt8(0x20, offset++);
                    buf.writeUInt8(geraetbereich, offset++);
                    buf.writeUInt8(adr, offset++);
                    buf.writeUInt8(adrzus, offset++);
                    buf.writeUInt8(adrerw, offset++);
                    buf.writeUInt8(zustand, offset++);
                }
                this.#SendeSpeicher.push(buf);
                break;
            case 0x24:
            ...
            case 0x55:
                buf = Buffer.alloc(16);
                buf.writeUInt8(0x02, offset++);
                buf.writeUInt8(0x55, offset++);
                buf.writeUInt8(geraetbereich, offset++);
                buf.writeUInt8(0x02, offset++);
                buf.writeUInt8(0x03, offset++);
                buf.writeUInt8(0x10, offset++);
                buf.writeUInt8(0x11, offset++);
                buf.writeUInt8(0x20, offset++);
                buf.writeUInt8(0x40, offset++);
                buf.writeUInt8(0x41, offset++);
                buf.writeUInt8(0x50, offset++);
                buf.writeUInt8(0x51, offset++);
                buf.writeUInt8(0x54, offset++);
                buf.writeUInt8(0x55, offset++);
                buf.writeUInt8(0x56, offset++);
                buf.writeUInt8(0x61, offset++);
                this.#SendeSpeicher.push(buf);
                break;
            default:
                buf = Buffer.alloc(4, 0);
                ...
                buf.writeUInt8(0x11, offset++);
                buf.writeUInt8(geraetbereich, offset++);
                buf.writeUInt8(0x2, offset++);   //nicht erfüllbar
                this.#SendeSpeicher.push(buf);
        }
    }

    //Länge anpassen auf 16 teilbar
    ...
    //Länge , Prüfsumme und Verschlüsselung
    ...
    #pruefeTelegrammZaehler() {
        ...
        if (this.#isEqual(this.#RC_rec, this.#TC - 1)) {
            if (this.#SendeZaehler > 3) {
                this.#logging('3 Wiederholungen', 'warn');
                this.#controller(_action.Disconnect);
                return false;
            }
            this.#logging('Datensatz wiederholen', 'warn');
            this.#controller(_action.Wiederholung);

            return false;
        }
        else if (this.#isEqual(this.#RC_rec, this.#TC)) {  //neue Meldung
                return true;
        } else
             {
        ...
        while (offset < data.length) {
            ...
            switch (typ) {
                ...
                case 0x40:  //Testmeldung
                    ...
                    offset += sl;
                    this.#SendeSpeicher.push(this.#getTestmeldungQuittungBuffer());
                    break;
                ...
                case 0x51:  //Herstelleridentifikation
                    ...
                    vds[Satz].Value = data.toString('utf-8', offset, offset + sl);
                    offset += sl;
                    break;
                ...
                case 0x56:  //Identnummer
                    ...
                    if (!this.#Id) {
                        ...
                        this.emit('connect', vds[Satz].Value);
                        let obj = this.#GetDevice(vds[Satz].Value, null);
                        if (obj) {
                            ...
                            this.#IdentnummerPruefen(vds[Satz].Value);
                        } else {
                            this.#logging(`Unbekannte Identnummer:${vds[Satz].Value}`, 'warn');
                        }
                    }
                    offset += sl;
                    break;
    ...
    */
    ...
    //Prüft die empfangene Identnummer mit der hinterlegten Identnummer
